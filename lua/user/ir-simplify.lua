...
-- Version: 6.6 - Fix insertvalue Indices (Literal vs Variable Collision)
...
local function simplify(var, vars, memo, depth)
  ...
  local function get_ref_or_val_structural(operand_var, is_literal_index)
      ...
      -- Table Protection (Metadata)
      if type(operand_var) == "table" then return operand_var end
      
      -- [FIX] Literal Indices (like "2" in insertvalue) should NOT be looked up as vars
  ...
  if arithmetic_ops[node.op] then
      ...
      -- Structural Logic
      -- [FIX] insertvalue 的 extra (索引) 必须被视为 Literal，否则 "2" 会被当成变量 %2
      local is_insert_idx = (node.op == "insertvalue")
      ...
      -- 对于 insertvalue, extra 是索引，传入 true
      local extra = get_ref_or_val_structural(node.extra, is_insert_idx)
...
function M.show_deps()
  ...
  local lines = {
    "╔═══════════════════════════════════════════════════════════╗",
    "║        🔍 LLVM IR Hybrid View (V6.6 Fixed)               ║",
    "╚═══════════════════════════════════════════════════════════╝",
    "",
