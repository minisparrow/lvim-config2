...
-- Version: 4.1 - Keep original variable names, selective parameter extraction
-- Author: minisparrow
-- Date: 2025-11-18
...
-- Check if a value should be kept as variable reference
local function should_keep_as_var(val)
  -- Keep as variable if it's a string that looks like a variable name (number or identifier)
...
-- Convert value to string for display, keeping original variable names
local function value_to_string(val, depth, vars, memo)
  depth = depth or 0
  ...
  elseif val.op == "select" then
    local cond_str = value_to_string(val.left, depth+1, vars, memo)
    local true_str = value_to_string(val.right, depth+1, vars, memo)
    local false_str = value_to_string(val.extra, depth+1, vars, memo)
    return string.format("select(%s ? %s : %s)", cond_str, true_str, false_str)
  elseif val.op == "icmp" then
    local left_str = value_to_string(val.left, depth+1, vars, memo)
    local right_str = value_to_string(val.right, depth+1, vars, memo)
  ...
  elseif val.op == "getelementptr" then
    ...
    local base_str = value_to_string(val.left, depth+1, vars, memo)
    local idx_str = value_to_string(val.right, depth+1, vars, memo)
    return string.format("getelementptr(%s, %s)", base_str, idx_str)
  elseif val.op == "ldmatrix" then
    -- Keep parameter as variable reference
    local arg_str = value_to_string(val.left, depth+1, vars, memo)
    return string.format("ldmatrix(%s)", arg_str)
  elseif val.op == "extractvalue" then
    local agg_str = value_to_string(val.left, depth+1, vars, memo)
    return string.format("extractvalue(%s[%s])", agg_str, tostring(val.right))
  elseif val.op == "insertvalue" then
    local agg_str = value_to_string(val.left, depth+1, vars, memo)
    local val_str = value_to_string(val.right, depth+1, vars, memo)
  ...
  elseif val.op == "insertelement" then
    local vec_str = value_to_string(val.left, depth+1, vars, memo)
    local val_str = value_to_string(val.right, depth+1, vars, memo)
...
local function simplify(var, vars, memo, depth)
  ...
  -- Handle structural operations - keep variable references for certain operations
  if node.op == "getelementptr" then
    -- For getelementptr, simplify base but keep index as variable reference if possible
  ...
  if node.op == "ldmatrix" then
    -- For ldmatrix, keep the argument as variable reference
  ...
  log(string.format("  %%%s: left=%s, right=%s", var, value_to_string(left, 0, vars, memo), value_to_string(right or "nil", 0, vars, memo)))
  ...
  if node.op == "add" then
    ...
    elseif right == 0 then
      ...
      local result = string.format("(%s + %s)", value_to_string(left, 0, vars, memo), value_to_string(right, 0, vars, memo))
      memo[var] = result
      return result
    end
  ...
  elseif node.op == "fadd" then
    ...
    elseif right == 0 or right == 0.0 then
      ...
      local result = string.format("(%s +f %s)", value_to_string(left, 0, vars, memo), value_to_string(right, 0, vars, memo))
      memo[var] = result
      return result
    end
  ...
  local result = string.format("unknown_op_%s(%s, %s)", node.op, value_to_string(left, 0, vars, memo), value_to_string(right or "", 0, vars, memo))
  memo[var] = result
  return result
end
...
local function build_dependency_tree(var, vars, memo, visited, indent, lines, path)
  ...
  local result_str = value_to_string(result, 0, vars, memo)
  ...
  elseif node.op == "extractvalue" or node.op == "extractelement" then
    if node.left and vars[node.left] then
      table.insert(lines, indent .. "└─ operand:")
      build_dependency_tree(node.left, vars, memo, visited, last_indent, lines, new_path)
    end
