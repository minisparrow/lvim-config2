...
-- Version: 6.1 - Inline Expressions with Full Tree Expansion
...
-- [MODIFIED] Value to String (Supports Cached Strings)
local function value_to_string(val, depth)
  depth = depth or 0
  if depth > 10 then return "..." end
  
  -- 1. 优先返回预计算好的内联字符串 (Arithmetic Inline)
...
local function simplify(var, vars, memo, depth)
  ...
  -- 4. Identity Simplification (返回 child result)
  -- 注意：这里返回 left/right，实际上是返回了 child 的 simplified object。
  -- build_tree 依然会根据 var 查找 vars[var] 来构建树，所以树结构不会丢失，但显示的值会是化简后的。
  ...
  -- 5. [KEY] Arithmetic Inlining
  -- 返回一个 Table，但附带 simplified_str。
  -- 这样 value_to_string 会显示字符串，但 build_tree 会看到 Table 并继续展开。
  if arithmetic_ops[node.op] then
      local symbol = arithmetic_ops[node.op]
      
      -- 辅助：将操作数转为字符串用于拼接
      local function op_to_str(val, original_var)
          -- 如果子节点也是算术节点，并且有内联字符串，直接嵌入！(实现 ((a+b)+c))
...
local function build_dependency_tree(var, vars, memo, visited, indent_prefix, child_prefix, lines, path, label)
    ...
    -- [MODIFIED] 现在即使 result 是 table (包含 simplified_str)，我们也继续展开
    -- 只有当 result 既不是 table，也不是我们自定义的 node 结构时才停止
...
function M.show_deps()
  ...
  local lines = {
    "╔═══════════════════════════════════════════════════════════╗",
    "║        🔍 LLVM IR C-Style Tree (Full Expansion)          ║",
    "╚═══════════════════════════════════════════════════════════╝",
    "",
