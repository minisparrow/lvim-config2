...
-- Version: 5.6 - Fix Global Symbol (addressof) & Regex
...
local function parse_line(line)
  ...
  -- [FIXED] AddressOf: 增强正则，支持 @foo.bar 或 @foo-bar
  -- 旧正则 @([%w_]+) 无法匹配带点号的符号，且 logic 中参数位置需在 build_tree 修正
...
local function build_tree()
  ...
  for i, line in ipairs(lines) do
    ...
    if var then
      ...
      elseif op == "tid.x" or op == "undef" then 
        vars[var] = create_node(op, nil, nil, op, nil, type_info)
      -- [FIXED] Addressof Special Case
      -- parse_line 返回的 arg1 是符号名，对于 addressof，必须将其存入 `value` 字段
      elseif op == "addressof" then
        vars[var] = create_node("addressof", nil, nil, arg1, nil, type_info)
      else
...
local function value_to_string(val, depth)
  ...
  if op == "addressof" then 
    -- 这里 val.value 应该存储符号名 (如 global_smem)
    return string.format("llvm.mlir.addressof @%s%s", tostring(val.value), fmt_type())
...
-- ==========================================================
-- Simplify Logic (Algebraic Simplification)
-- ==========================================================
local function simplify(var, vars, memo, depth)
  ...
  -- [FIXED] Addressof 直接返回，无需 resolve 操作数
  if node.op == "addressof" then 
      memo[var] = {op="addressof", value=node.value, type_info=node.type_info}
      return memo[var] 
  end
  ...
  if left == right and left ~= nil then
      if node.op == "and" or node.op == "or" then memo[var] = left; return left end
      if node.op == "xor" or node.op == "sub" then memo[var] = 0; return 0 end
  end
