...
-- Version: 4.0 - Added parameter extraction and split window display
-- Author: minisparrow
-- Date: 2025-11-18
...
-- Configuration
M.config = {
  -- Enable parameter extraction for complex instructions
  extract_params = true,
...
local temp_var_counter = 0
local function gen_temp_var()
  temp_var_counter = temp_var_counter + 1
  return "tmp_" .. temp_var_counter
end

-- Check if an expression is complex enough to extract
local function is_complex_expr(expr_type, arg1, arg2, extra)
  if not M.config.extract_params then
    return false
  end
  
  -- Extract parameters for these operations if they contain nested operations
  local extractable_ops = {
    insertvalue = true,
    extractvalue = true,
    getelementptr = true,
    insertelement = true,
  ...
  if not extractable_ops[expr_type] then
    return false
  end
  
  -- Check if arguments are variables (not constants)
  local has_var = false
  if arg1 and type(arg1) == "string" and arg1:match("^%w+$") then
    has_var = true
  end
  if arg2 and type(arg2) == "string" and arg2:match("^%w+$") then
    has_var = true
  end
...
-- Convert value to string for display with optional parameter extraction
local function value_to_string(val, depth, extracted_vars)
  depth = depth or 0
  ...
  elseif val.op == "select" then
    return string.format("select(%s ? %s : %s)", 
      value_to_string(val.left, depth+1, extracted_vars), 
      value_to_string(val.right, depth+1, extracted_vars), 
      value_to_string(val.extra, depth+1, extracted_vars))
  elseif val.op == "icmp" then
    return string.format("icmp_%s(%s, %s)", 
      tostring(val.extra), 
      value_to_string(val.left, depth+1, extracted_vars), 
      value_to_string(val.right, depth+1, extracted_vars))
  elseif val.op == "getelementptr" then
    local base_str = value_to_string(val.left, depth+1, extracted_vars)
    local idx_str = value_to_string(val.right, depth+1, extracted_vars)
    
    -- Extract complex parameters if enabled
    if M.config.extract_params and depth == 0 then
      if type(val.left) == "table" and val.left.op then
        local temp_var = gen_temp_var()
        extracted_vars[temp_var] = base_str
        base_str = "%" .. temp_var
      end
      if type(val.right) == "table" and val.right.op then
        local temp_var = gen_temp_var()
        extracted_vars[temp_var] = idx_str
        idx_str = "%" .. temp_var
      end
    end
  ...
  elseif val.op == "ldmatrix" then
    return string.format("ldmatrix(%s)", value_to_string(val.left, depth+1, extracted_vars))
  elseif val.op == "extractvalue" then
    local agg_str = value_to_string(val.left, depth+1, extracted_vars)
    
    if M.config.extract_params and depth == 0 and type(val.left) == "table" and val.left.op then
      local temp_var = gen_temp_var()
      extracted_vars[temp_var] = agg_str
      agg_str = "%" .. temp_var
    end
  ...
  elseif val.op == "insertvalue" then
    local agg_str = value_to_string(val.left, depth+1, extracted_vars)
    local val_str = value_to_string(val.right, depth+1, extracted_vars)
    ...
    if M.config.extract_params and depth == 0 then
      ...
      if type(val.right) == "table" and val.right.op then
        local temp_var = gen_temp_var()
        extracted_vars[temp_var] = val_str
        val_str = "%" .. temp_var
      end
    end
    
    return string.format("insertvalue(%s, [%s] = %s)", agg_str, tostring(val.extra), val_str)
  elseif val.op == "bitcast" then
    local type_info = (type(val.extra) == "table" and val.extra.to) or "?"
    return string.format("bitcast(%s -> %s)", value_to_string(val.left, depth+1, extracted_vars), type_info)
  elseif val.op == "insertelement" then
    local vec_str = value_to_string(val.left, depth+1, extracted_vars)
    local val_str = value_to_string(val.right, depth+1, extracted_vars)
    
    if M.config.extract_params and depth == 0 then
      if type(val.left) == "table" and val.left.op then
        local temp_var = gen_temp_var()
        extracted_vars[temp_var] = vec_str
        vec_str = "%" .. temp_var
      end
      if type(val.right) == "table" and val.right.op then
        local temp_var = gen_temp_var()
    ...
    return string.format("insertelement(%s, [%s] = %s)", vec_str, tostring(val.extra or "?"), val_str)
  elseif val.op == "extractelement" then
    ...
    return string.format("extractelement(%s[%s])", agg_str, tostring(val.right))
  end
...
local function build_dependency_tree(var, vars, memo, visited, indent, lines, path)
  ...
  local result_str = value_to_string(result, 0, extracted_vars)
  ...
  if next(extracted_vars) then
    table.insert(lines, indent .. "  (with extracted parameters:)")
    for temp_var, temp_expr in pairs(extracted_vars) do
      table.insert(lines, indent .. "    %" .. temp_var .. " = " .. temp_expr)
    end
  end
...
function M.show_deps()
  ...
  if next(extracted_vars) then
    table.insert(lines, "┌─────────────────────────────────────────────────────────┐")
    table.insert(lines, "│ 📦 EXTRACTED PARAMETERS                                 │")
    table.insert(lines, "└─────────────────────────────────────────────────────────┘")
    table.insert(lines, "")
    for temp_var, temp_expr in pairs(extracted_vars) do
      table.insert(lines, string.format("  %%%s = %s", temp_var, temp_expr))
    end
    table.insert(lines, "")
  end
  ...
  table.insert(lines, "")
  table.insert(lines, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
  table.insert(lines, "  Shortcuts: [Q]uit  [Y]ank result  [D]ebug  [E]xtract params toggle")
  table.insert(lines, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
  ...
  local keymaps = {
    ...
    { 'n', 'Y', string.format(':let @+ = "%s"<CR>:let @" = "%s"<CR>:echo "Result copied!"<CR>', 
      yank_text:gsub('"', '\\"'), yank_text:gsub('"', '\\"')), 'Yank result' },
    { 'n', 'e', ':LLVMToggleExtract<CR>:close<CR>:LLVMDeps<CR>', 'Toggle extract params' },
    { 'n', 'E', ':LLVMToggleExtract<CR>:close<CR>:LLVMDeps<CR>', 'Toggle extract params' },
  }
...
-- Toggle parameter extraction
function M.toggle_extract()
  M.config.extract_params = not M.config.extract_params
  vim.notify("LLVM Simplifier parameter extraction: " .. (M.config.extract_params and "ON" or "OFF"), vim.log.levels.INFO)
end
...
function M.setup(user_config)
  ...
  vim.api.nvim_create_user_command('LLVMToggleExtract', M.toggle_extract, {})
  ...
  vim.notify("🎯 LLVM Simplifier v4.0 loaded! (by @minisparrow)\n" ..
    "  <leader>ld or :LLVMDeps - Show dependency tree\n" ..
    "  :LLVMDebug - Toggle debug mode\n" ..
    "  :LLVMToggleExtract - Toggle parameter extraction\n" ..
    "  :LLVMToggleWindow - Toggle split/float window\n" ..
    "  New: Parameter extraction for complex instructions\n" ..
    "  New: Split window display (below original code)\n" ..
    "  Inside window: y=copy, q=quit, e=toggle extract", 
    vim.log.levels.INFO)
end
