...
-- Author: minisparrow
-- Date: 2025-11-11
-- Version: 2.1 - Added insertvalue support
...
local function format_insertvalue(aggregate, value, indices)
  -- If we're building an array/vector, format it nicely
  if type(aggregate) == "table" and aggregate.type == "insertvalue_chain" then
    -- Continue the chain
    local new_chain = {type = "insertvalue_chain", values = {}}
    for _, v in ipairs(aggregate.values) do
      table.insert(new_chain.values, v)
    end
    table.insert(new_chain.values, {value = value, indices = indices})
    return new_chain
  else
    -- Start a new chain
    return {
      ...
      base = aggregate,
      values = {{value = value, indices = indices}}
    }
  end
end

-- Convert insertvalue chain to string
local function insertvalue_chain_to_string(chain)
  if type(chain) ~= "table" or chain.type ~= "insertvalue_chain" then
    return tostring(chain)
  end
  
  -- Format as array construction
  local parts = {}
  for _, item in ipairs(chain.values) do
    table.insert(parts, string.format("[%s]=%s", item.indices, tostring(item.value)))
  end
  
  local base_str = chain.base == "undef" and "{}" or tostring(chain.base)
  
  -- If there are many insertions, show it compactly
  if #parts > 4 then
    return string.format("insertvalue_chain{base=%s, %d insertions}", base_str, #parts)
  else
    return string.format("insertvalue{%s, %s}", base_str, table.concat(parts, ", "))
  end
end
...
local function result_to_string(result)
  if type(result) == "table" and result.type == "insertvalue_chain" then
    return insertvalue_chain_to_string(result)
  else
...
function M.simplify()
  ...
  local result_str = result_to_string(result)
  ...
  for _, var in ipairs(chain) do
    if memo[var] then
      local var_result = result_to_string(memo[var])
...
function M.setup()
  ...
  -- Add keybindings
  lvim.keys.normal_mode["<leader>irf"] = ":LLVMSimplify<CR>"     -- Full details
  lvim.keys.normal_mode["<leader>irr"] = ":LLVMResult<CR>"       -- Result only
  lvim.keys.normal_mode["<leader>irp"] = ":LLVMPreview<CR>"      -- Command line preview
  
  vim.notify("🎯 LLVM Simplifier v2.1 loaded! (by @minisparrow)\n" ..
    "  <leader>irf - Full analysis\n" ..
    "  <leader>irr - Quick result\n" ..
    "  <leader>irp - CLI preview\n" ..
    "  New: insertvalue support!\n" ..
