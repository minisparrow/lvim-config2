...
-- Version: 6.2 - Structural References & C-Style Tree
...
local function value_to_string(val, depth)
  ...
  elseif val.left and val.right then
      return string.format("(%s %s %s)", fmt(val.left), op, fmt(val.right))
  end
...
-- ==========================================================
-- Simplify Logic (Mixed Reference/Value)
-- ==========================================================
local function simplify(var, vars, memo, depth)
  ...
  -- 2. Resolve Children (Recursively simplify)
  ...
  -- 5. Arithmetic Inlining
  -- If arithmetic, we construct a "simplified_str" so it can be inlined by parent arithmetic nodes.
  ...
  -- 6. [NEW] Structural Nodes: Use References for Non-Simple Operands
  -- For GEP, InsertValue, etc., we want to see "%2" not the full expansion "getelementptr(...)"
  -- This keeps the top-level view clean.
  
  local function use_ref_or_val(val, original_var_name)
      -- Keep simple values (numbers, bools, atoms)
      if type(val) == "number" or type(val) == "boolean" then return val end
      if val == "tid.x" or val == "undef" then return val end
      if type(val) == "table" and val.op == "addressof" then return val end -- Keep @global
      
      -- For complex tables (Structural) or inlined strings (Arithmetic), use the Variable Name
      if original_var_name then
          return original_var_name -- Returns "2" or "38"
      end
      return val
  end
  ...
  memo[var] = {
      op = node.op,
      left = use_ref_or_val(left, node.left),   -- Pass "2" instead of table
      right = use_ref_or_val(right, node.right), -- Pass "38" instead of string
      extra = node.extra,
      value = node.value,
...
local function build_dependency_tree(var, vars, memo, visited, indent_prefix, child_prefix, lines, path, label)
    ...
    -- Arithmetic Children (Expanded to show details)
