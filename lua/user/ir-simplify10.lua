...
-- Version: 3.1 - Added select operation support
-- Author: minisparrow
-- Date: 2025-11-13
...
local function value_to_string(val, depth)
  ...
  elseif val.op == "select" then
    return string.format("select(%s ? %s : %s)", value_to_string(val.left, depth+1), value_to_string(val.right, depth+1), value_to_string(val.extra, depth+1))
  elseif val.op == "icmp" then
    return string.format("icmp_%s(%s, %s)", tostring(val.extra), value_to_string(val.left, depth+1), value_to_string(val.right, depth+1))
...
local function build_dependency_tree(var, vars, memo, visited, indent, lines)
  ...
  if node.op == "select" then
    ...
    build_dependency_tree(node.right, vars, memo, visited, new_indent, lines)
    table.insert(lines, indent .. "└─ false_value:")
    build_dependency_tree(node.extra, vars, memo, visited, last_indent, lines)
...
function M.setup()
  ...
  vim.notify("🎯 LLVM Simplifier v3.1 loaded! (by @minisparrow)\n" ..
    "  <leader>ld or :LLVMDeps - Show dependency tree\n" ..
    "  :LLVMDebug - Toggle debug mode\n" ..
    "  New: select and icmp support!\n" ..
