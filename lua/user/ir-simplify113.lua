...
-- Version: 5.4 - Fix extractvalue Index Collision (Literal vs Variable)
...
local function value_to_string(val, depth)
  ...
  elseif op == "extractvalue" then
    -- [NOTE] val.right 这里是索引，直接转字符串，不要格式化
...
local function simplify(var, vars, memo, depth)
  ...
  -- [CRITICAL FIX] 区分 "变量" 和 "字面量索引"
  local left = resolve(node.left)
  
  local right
  if node.op == "extractvalue" then
      -- extractvalue 的 right 是字面量索引 (如 "3")，绝对不能解析为变量 "%3"
      right = node.right 
  else
