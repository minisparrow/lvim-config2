...
-- Version: 4.3 - Use standard LLVM IR syntax for insertvalue/extractvalue
-- Author: minisparrow
-- Date: 2025-11-18
...
local function value_to_string(val, depth)
  ...
  elseif val.op == "insertelement" then
    ...
    local val_str = type(val.right) == "string" and ("%" .. val.right) or value_to_string(val.right, depth+1)
    return string.format("insertelement(%s, %s[%s])", vec_str, val_str, tostring(val.extra or "?"))
...
function M.setup(user_config)
  ...
  vim.notify("🎯 LLVM Simplifier v4.3 loaded! (by @minisparrow)\n" ..
    "  <leader>ld or :LLVMDeps - Show dependency tree\n" ..
    "  :LLVMDebug - Toggle debug mode\n" ..
    "  :LLVMToggleWindow - Toggle split/float window\n" ..
    "  Feature: Standard LLVM IR syntax for insertvalue/extractvalue\n" ..
